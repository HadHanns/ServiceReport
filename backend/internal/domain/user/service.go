...
import (
	...
	"github.com/company/internal-service-report/pkg/passwordutil"
	"gorm.io/gorm"
)
...
func (s *Service) ResetPassword(ctx context.Context, targetID uint64, requesterID uint64) (string, error) {
	...
	temp, err := passwordutil.GenerateTempPassword(12)
	if err != nil {
		return "", err
	}
	...
	if err := s.db.WithContext(ctx).Model(&User{}).Where("id = ?", targetID).Update("password_hash", hash).Error; err != nil {
		return "", err
	}
	if s.mailer != nil {
		body := fmt.Sprintf(
			"Hello %s,\n\nYour password has been reset by an administrator.\n\nTemporary password: %s\n\nPlease sign in and change it immediately.\n",
			target.FullName, temp,
		)
		if err := s.mailer.Send(target.Email, "Temporary Password Reset", body); err != nil {
			return "", err
		}
	}
	return temp, nil
...
func (s *Service) ListTeknisi(ctx context.Context) ([]User, error) {
